...
namespace WindowsFilesVanguard
{
    public partial class WFV_MainForm : Form
    {
        ...
        public void RunProgressBar(string progressLabel, int maxProgress, Action<object, EventArgs> action, Action<object, EventArgs> postAction = null)
        {
            ...
            WFV_Core.progressForm = new WFV_Progress( progressLabel, maxProgress, action, postAction);
        ...
        private void TerminateIfNeeded()
        {
            if (rbExecuteOtherProgram.Checked || rbExecuteWith.Checked || rbExecuteCorruptedFile.Checked)
                if (WFV_Core.currentTargetType == "File" && cbTerminateOnReExec.Checked && WFV_Executor.otherProgram != null)
                {
                    string otherProgramShortFilename = WFV_Executor.otherProgram.Substring(WFV_Executor.otherProgram.LastIndexOf(@"\") + 1);
